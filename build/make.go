...
var deployDir = filepath.Join(deploy, jsonReport)
...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", jsonReport, getPluginVersion(), getGOOS(), getArch())
...
func compileGoPackage(packageName string) {
	runProcess("go", "build", "-o", getGaugeExecutablePath(jsonReport))
}
...
func copyPluginFiles(destDir string) {
	...
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), jsonReport+".exe")] = bin
	} else {
		files[filepath.Join(getBinDir(), jsonReport)] = bin
	}
...
func installPlugin(installPrefix string) {
	...
	pluginInstallPath := filepath.Join(installPrefix, jsonReport, getPluginVersion())
